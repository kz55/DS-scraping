...
#'
#' @title 
#' Returns 500 recent tweets from a given user
#'
#' @description
#' \code{getTimeline} connects to the REST API of Twitter and returns
#' 500 recent tweets sent by these user.
#'
#' @author
#' Pablo Barbera \email{pablo.barbera@@nyu.edu} (modified)
#'
...
getTimeline <- function(filename, n=500, oauth, screen_name=NULL, 
    ...
    while (limit==0){
        Sys.sleep(sleep)
        # sleep for 5 minutes if limit rate is less than 250
    ...
    if (!is.null(screen_name)){
        params <- list(screen_name = screen_name, count=100, trim_user=trim_user)
    }
    if (!is.null(id)){
        params <- list(id=id, count=100, trim_user=trim_user)   
    }
    if (!is.null(since_id)){
    ...
    while (tweets < n & max_id != max_id_old & 
        ...
        max_id_old <- max_id
        if (!is.null(screen_name)){
            params <- list(screen_name = screen_name, count=100, max_id=max_id,
                trim_user=trim_user)
        }
        if (!is.null(id)){
            params <- list(id=id, count=100, max_id=max_id, trim_user=trim_user)
        }
        if (!is.null(since_id)){
            params[['since_id']] <- since_id
        }
